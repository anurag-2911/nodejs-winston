...
var File = exports.File = function (options) {
  ...
  this.maxsize     = options.maxsize || false;
...
File.prototype._createStream = function () {
  ...
  (function checkFile (target) {
    ...
    function createAndFlush (size) {
      ...
      // self.emit('error',new Error('Whoops!'));
    ...
    //     // console.log('self '+ self._archive);
    //     var out = fs.createWriteStream(self._archive + '.zip');

    //     inp.pipe(gzip).pipe(out);
    //     // console.log('about to compress');
               
    //     fs.unlink(String(self._archive), (err)=>{
    //     if(err)
    //     {
    ...
    //     else
    //     {
    //       // console.log('file deletion sucessful: ' + self._archive);
    //     }}); 
    ...
    fs.stat(fullname, function (err, stats) {
      // console.log('checking stats '+fullname);
      // console.trace();
      if (err) {
        if (err.code !== 'ENOENT') {
...
File.prototype._incFile = function (callback) {
  ...
  if (!this.tailable) {
    this._created += 1;
    // console.log('_checkMaxFilesIncrementing');
    this._checkMaxFilesIncrementing(ext, basename, callback);
  }
  else {
    // console.log('checking max files tailable');
...
File.prototype._checkMaxFilesIncrementing = function (ext, basename, callback) {
  ...
  target = path.join(self.dirname, basename + (oldest !== 0 ? oldest : '') + ext +
    (self.zippedArchive ? '.zip' : ''));
  // console.log('file deleted is '+target);
...
File.prototype._checkMaxFilesTailable = function (ext, basename, callback) {
  ...
  for (var x = this.maxFiles - 1; x > 0; x--) {
    ...
    //   return function (cb) {
        var tmppath = path.join(self.dirname, basename + (x - 1) + ext +(self.zippedArchive ? '.zip' : ''));
...

        {
          ...
          //   return cb(null);
          // }
          // console.log('oldpath>' + tmppath);
          // console.log('newpath '+ path.join(self.dirname, basename + i + ext +
          //   (self.zippedArchive ? '.zip' : '')));
          fs.rename(tmppath, path.join(self.dirname, basename + x + ext +(self.zippedArchive ? '.zip' : '')), (error)=>{
            if(error)
            {
  ...
  if (self.zippedArchive) {
    ...
    var oldpath=path.join(self.dirname, basename + ext);
    var newPath =path.join(self.dirname, basename + 1 + ext);
    // console.log('1-oldpath '+oldpath);
    // console.log('1-newpath '+ newPath);
    ...
    // console.log('self archive in zipped archive check '+self._archive);
    // var oldpath=path.join(self.dirname, basename + ext);
    // var newPath =path.join(self.dirname, basename + 1 + ext);
    // console.log('1-oldpath '+oldpath);
    // console.log('1-newpath '+ newPath);
