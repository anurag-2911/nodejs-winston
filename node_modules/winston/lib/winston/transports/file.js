...
File.prototype._checkMaxFilesIncrementing = function (ext, basename, callback) {
  ...
  target = path.join(self.dirname, basename + (oldest !== 0 ? oldest : '') + ext +(self.zippedArchive ? '.zip' : ''));
...
File.prototype._checkMaxFilesTailable = function (ext, basename, callback) {
  ...
  for (var x = this.maxFiles - 1; x > 0; x--) {
        ...
        {
            fs.renameSync(tmppath, path.join(self.dirname, basename + x + ext +(self.zippedArchive ? '.zip' : '')));
        }
      };
  ...
  if (self.zippedArchive) {
    ...
    if (self._archive) {
      ...
      fs.unlinkSync(String(self._archive));
